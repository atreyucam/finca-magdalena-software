...
export default function DetalleCosecha() {
  ...
  const handleCerrar = async () => {
    ...
    const fecha = window.prompt(
      "Confirmar fecha de cierre (YYYY-MM-DD):",
  ...
  if (!cosecha)
    return <div className="p-10 text-center text-rose-500">Ciclo no encontrado</div>;

  const esCerrada = cosecha.estado === "Cerrada";

  // ✅ Opción A: pendientes = total - finalizadas (finalizadas = verificadas + canceladas)
  const totalTareas = cosecha.metricas?.totalTareas ?? 0;
  const verificadas = cosecha.metricas?.verificadas ?? 0;
  const canceladas = cosecha.metricas?.canceladas ?? 0;
  const finalizadas = cosecha.metricas?.finalizadas ?? (verificadas + canceladas);
  const pendientes = Math.max(0, totalTareas - finalizadas);
    ...
    <section className="-m-4 sm:-m-6 lg:-m-8 bg-slate-50 min-h-screen p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-[1200px] space-y-6">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-8">
            <div className="space-y-3">
              ...
              <p className="font-bold">
                Estado:{" "}
                <Badge variante={esCerrada ? "neutro" : "activo"}>
                  {cosecha.estado.toUpperCase()}
                </Badge>
              </p>

              <div className="flex flex-col gap-2 items-start p-2">
                <p className="text-xs font-bold bg-slate-100 px-2 py-1 rounded text-slate-700">
                  CÓDIGO: {cosecha.codigo}
                </p>
                <p className="text-xl font-mono text-slate-700 font-medium">
                  {cosecha.anio_agricola_label}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col items-end justify-center gap-4">
              <div className="text-right space-y-1">
                {/* ✅ cambia el texto: ahora es Finalización */}
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  Finalización de labores (100% requerido)
                </p>
                  ...
                  <div className="w-48 h-2.5 bg-slate-100 rounded-full overflow-hidden border border-slate-50">
                    <div
                      className={`h-full transition-all duration-700 ${
                        cosecha.metricas.puedeCerrar ? "bg-emerald-500" : "bg-amber-500"
                      }`}
                      style={{ width: `${cosecha.metricas.progresoVerificacion}%` }}
                    />
                  </div>
                  <span className="text-sm font-black text-slate-700">
                    {cosecha.metricas.progresoVerificacion}%
                  </span>
                </div>
              </div>
              ...
              {!esCerrada && (
                ...
                >
                  {cosecha.metricas.puedeCerrar
                    ? "Cerrar Ciclo Productivo"
                    : "Cierre Bloqueado (Tareas no finalizadas)"}
                </Boton>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                {cosecha.PeriodoCosechas?.map((p, idx) => (
                  <div
                    key={p.id}
                    className="flex items-center justify-between p-5 rounded-2xl border border-slate-100 bg-slate-50 hover:bg-white transition-colors"
                  >
          ...
          {/* ✅ Resumen de Control (con canceladas) */}
          <div className="rounded-2xl border border-slate-200 bg-slate-900 p-6 text-white">
            <h3 className="text-xs font-bold uppercase opacity-40 mb-8 tracking-widest">
              Resumen de Control
            </h3>
            ...
            <div className="space-y-8">
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-3xl font-black">{totalTareas}</span>
              </div>

              <div className="flex justify-between items-baseline">
                <span className="text-sm opacity-60">Verificadas</span>
                <span className="text-3xl font-black text-emerald-400">{verificadas}</span>
              </div>

              <div className="flex justify-between items-baseline">
                <span className="text-sm opacity-60">Canceladas</span>
                <span className="text-3xl font-black text-rose-300">{canceladas}</span>
              </div>
              ...
              <div className="flex justify-between items-baseline pt-4 border-t border-white/10">
                ...
                <span className="text-3xl font-black text-amber-500">{pendientes}</span>
              </div>
            </div>
            ...
            {!cosecha.metricas.puedeCerrar && !esCerrada && (
              <div className="mt-10 p-4 rounded-xl bg-white/5 border border-white/10 flex gap-3 items-start">
                ...
                <p className="text-[10px] leading-relaxed opacity-60 font-medium">
                  Para cumplir con la trazabilidad técnica, no se permite el cierre mientras existan
                  tareas en estado Por hacer / En curso / Lista. Las tareas Verificadas y Canceladas
                  cuentan como finalizadas.
                </p>
              </div>
            )}
          </div>
