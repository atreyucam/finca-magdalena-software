...
import useAuthStore from "../../store/authStore"; // <--- 1. Importamos el store de autenticación
...
export default function ModalSemanaHistorial({ open, nominaId, onClose }) {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setLoading(true);
      try {
        // 3. Agregamos el header Authorization
        const res = await fetch(`${apiUrl}/pagos/semana?nomina_id=${nominaId}`, {
          method: "GET",
          headers: { 
