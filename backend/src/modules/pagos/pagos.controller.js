...
exports.consolidarSemana = async (req, res, next) => {
    try {
        res.status(201).json(
            await service.consolidarSemana(req.user, req.body)
        );
    } catch (e) {
        next(e);
    }
...
exports.upsertDetalle = async (req, res, next) => {
    try {
        res.status(201).json(
            await service.upsertDetalle(+req.params.nominaId, req.body)
        );
    } catch (e) {
        next(e);
    }
